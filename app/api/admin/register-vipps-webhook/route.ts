...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (listRes.ok) {
      ...
      if (existing) {
        return NextResponse.json({
          ...
          note: 'Webhook fantes allerede mot denne URL-en — ingen ny opprettet, secret vises kun ved opprettelse.',
        });
      }
    } else {
    ...
    return NextResponse.json({ ok: true, created: true, id: created.id, secret: created.secret, events: EVENTS });
  } catch (error) {
